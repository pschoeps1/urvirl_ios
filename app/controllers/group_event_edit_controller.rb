class GroupEventEditController < UIViewController
  attr_accessor :event
  def viewDidLoad
    super
    ...
    rmq.append(UIView, :event_creation_form).tap do |q|
      ...
      q.append(UIButton, :event_edit_button).on(:tap) do |_|
  ...
  def handle_valid_submission
    ...
    process_edit_event @event_name.text, start_at, end_at, @event_content.text
  end

  def process_edit_event(name, start_at, end_at, content)
    auth_token = MotionKeychain.get('auth_token')
    group_id = @group
    event_id = @event["id"]

    EventEditService.new(self, {name: name, start_at: start_at, end_at: end_at, content: content, group_id: group_id, event_id: event_id, auth_token: auth_token }).process
  end

  def handle_event_edit_failed
    App.alert('Event creation failed, please try again')
  end

  def handle_event_edit_successful
  ...
  def go_back
    new_controller = GroupEventViewController.alloc.init
